...
Very fist step to perform. Set of shell scripts that groups initial set of actions to deploy Moodle's infrastructure on Google Cloud.
...
Once the file store (PV) exists we can go ahead and claim for access to it. This is accomplished by the task executed under step 3.
...
Once both Moodle's image and infrastructure to host it are created, we can move forward and deploy it in GKE as a Helm Chart. This directory groups the tasks to do just it.
...
This final step does configure Cloud Load Balancer to properly serve the Moodle's cluster as Ingress.
